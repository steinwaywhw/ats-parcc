...
in 
	red (l +> b <+ o <+> (parser ()), lam x => TmLam (fst x, snd x))
end

implement parser_var () = 
	red (id (), lam x => TmVar x)
	where {
		val _ = show "var\n"
	}
...
implement parser_app () = 
	red (
		(parser ()) <+ ws () <+> between ((parser ()), litstring "(", litstring ")"),
...
implement parser () = 
	(parser_var ()) <|> (parser_lam ()) <|> (parser_app ()) <|> between ((parser ()), litstring "(", litstring ")")
	where {
		val _ = show "term\n"
	}
